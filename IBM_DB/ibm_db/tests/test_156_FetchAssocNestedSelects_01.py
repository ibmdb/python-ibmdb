...
class IbmDbTestCase(unittest.TestCase):
  ...
  def test_156_FetchAssocNestedSelects_01(self):
    ...
    self.obj.assert_expect(self.run_test_156)
  ...
  def run_test_156(self):
    ...
    while ( row ): 
        ...
        if (row['COMM'] == None):
            row['COMM'] = ''
        print("%s %s %s %s %s %s" % (row['ID'],row['NAME'],row['JOB'],row['YEARS'], row['SALARY'], row['COMM']))
        row = ibm_db.fetch_assoc(result)
    ...
    while ( row2 ):    
        if (row2['MGRNO'] == None):
            row2['MGRNO'] = ''
        print("%s %s %s %s %s" % (row2['DEPTNO'], row2['DEPTNAME'], row2['MGRNO'], row2['ADMRDEPT'], row2['LOCATION']))
        row2 = ibm_db.fetch_assoc(result2)
